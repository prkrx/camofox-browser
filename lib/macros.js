const MACROS = {
  ...
  '@jobstreet_search': (query) => `https://www.jobstreet.co.id/id/job-search/${encodeURIComponent(query || '')}-jobs/`,
  '@tokopedia_search': (query) => `https://www.tokopedia.com/search?st=product&q=${encodeURIComponent(query || '')}`,
  '@shopee_search': (query) => `https://shopee.co.id/search?keyword=${encodeURIComponent(query || '')}`,
  '@bukalapak_search': (query) => `https://www.bukalapak.com/products?search%5Bkeywords%5D=${encodeURIComponent(query || '')}`,
  '@blibli_search': (query) => `https://www.blibli.com/cari/${encodeURIComponent(query || '')}`,
  '@lazada_search': (query) => `https://www.lazada.co.id/catalog/?q=${encodeURIComponent(query || '')}`
};
